...
export class StoresInforComponent implements OnInit {
  ...
  @Input() public alipayShopId: string = '';

  @Input() widthNum: boolean = false;
  ...
  @Input() labelText: string = '选择门店';

  @Input() alipayShop: boolean = false;

  @Input() public ifStoresAll: boolean = true;

  @Input() public ifStoresAuth: boolean = false; //是否需要授权
  @Input() public className: string = ''; //class名
  @Input() public className2: string = ''; //class名
  @Output() public storeIdOutput = new EventEmitter(); //选中门店的id

  @Output() public storeListPush = new EventEmitter(); //所有门店信息
  @Input() public AllStoreBoolean: boolean = true; //全部门店展示
  ...
  getStoresInfor() {
    let self = this;
    this.AllStoreBoolean = !self.ifStoresAll ? false : true;
    let data = {
    ...
    this.storesInforService.selectStores(data).subscribe(
      (res: any) => {
        if (res.success) {
          ...
          //是否禁用
          if (self.ifStoresAuth) {
            //需要授权
            storeList.forEach(function(item: any) {
              item.nzDisabled = item.hasAuth === true ? false : true;
            });
          }
          if (self.ifStoresAll && self.boolean) {
            //需要全部门店
            let list = {
              storeId: '',
              branchName: '全部门店',
            ...
            if (this.alipayShop) {
              if (this.alipayShopId || this.storeId) {
                ...
                this.alipayShopId = this.storeList[0]
                  ? this.storeList[0].alipayShopId
                  : '';
                this.storeName = this.storeList[0]
                  ? this.storeList[0].branchName
                  : '';
                this.storeId = this.storeList[0]
                  ? this.storeList[0].storeId
                  : '';
              }
            } else {
              if (this.storeId) {
                for (let i = 0; i < this.storeList.length; i++) {
                  if (this.storeId == this.storeList[i].storeId) {
                    this.store = this.storeList[i];
                    this.storeName = this.storeList[i]
                      ? this.storeList[i].branchName
                      : '';
                  }
                }
              } else {
