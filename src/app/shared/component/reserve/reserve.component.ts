...
export class ReserveComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  moduleId: any;
  recordStoreList: any[] = []; //预约记录页门店列表
  recordStoreId: string; //预约记录页门店ID
  ifStoresAuth: any;
  ...
  pageSize: any = 10;
  countPage: any = 1;
  theadName: any[] = ['门店', '手艺人', '预约电话', '预约时间', '预约商品', '人数', '操作'];
  ...
  storeListPush(event: any) {
    this.recordStoreList = event.storeList ? event.storeList : [];
  }
  //门店id
  getStoreId(event: any) {
    let self = this;
    this.recordStoreId = event.storeId ? event.storeId : '';
    this.pageNo = 1;
    this.getReservationsRecords();
  }
}
